...
public class SolutionHashcode4 {
    class Pizza {
...
record PizScore(List<SolutionHashcode4.Pizza> list, int ingredients){};
...
//        }
      ...
      //  pizzaDeliverer(args[1]);
        var result = new SolutionHashcode4();
...
//            }). collect(Collectors.toList());
    ...
    for(int x=0;x<=l.size()-size;x++) {
        ...
        for (int y = 0; y < size; y++) {
            Pizza p = l.get(x+y);
            long countNew2 = p.getIngredients().stream()
                    .filter((s) -> !used.contains(s)).count();
            groupPoints += countNew2;
            used.addAll(p.getIngredients());
            b.add(p);
        }


        System.out.println("best "+ "%s of %s groupby %s ".formatted(x,l.size(),size)+" "+ groupPoints + " " +bestScore);

        if(groupPoints>bestScore) {
            bestScore= groupPoints;
...
if(t==bestScore){//found max score..short
    ...
    return new PizScore(best,bestScore);
        ...
        if(prevCount>(l.size()/5)){ //(l.size()/10)
