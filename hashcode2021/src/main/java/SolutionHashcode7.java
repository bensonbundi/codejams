...
public class SolutionHashcode7 {
...
//System.out.println (filePath);
    ...
    record PizScore(PriorityQueue<SolutionHashcode7.Pizza> list, int ingredients){};
...
//        }
        ...
        var result = new SolutionHashcode7();
    ...
    public PizScore best(List<Pizza> l,int size) {
        ...
        for(int x=0;x<=l.size()-size;x++) {
            ...
            for (int y = 0; y < size && y+x< l.size(); y++) {
                ...
                String k =  used.parallelStream().map(s -> s).reduce("",(s, s2) -> s+s2);
                List<Pizza> sorted =  bestScoreMap.get(k);
                if(sorted ==null ){

                    sorted= l.parallelStream().filter(pizza -> {
                        return !usedList.contains(pizza);
                    }).filter(pizza -> {
                        long countNew2 = pizza.getIngredients().parallelStream()
...
//                    return o2.getIngredients().size()-o1.getIngredients().size();
                ...
                sorted.stream().filter(pizza -> l.contains(pizza)).findAny().ifPresentOrElse(pizza -> {
                    ...
                    long countNew2 = pizza2.getIngredients().parallelStream()
                            .filter((s) -> !used.contains(s)).count();
                    p =null;
                    if(countNew>countNew2){
                        p=pizza;
            ...
            int t =b.parallelStream().mapToInt(value -> value.getIngredients().size()).sum();
