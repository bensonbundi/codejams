...
public class SolutionHashcode2 {
    class Pizza {
...
//        }
      ...
      //  pizzaDeliverer(args[1]);
        var result = new SolutionHashcode2();
        ...
        for(int x=list.size()-1;x>=0;x--){
          ...
          //  list.stream().limit(5).forEach(pizza -> System.out.println(pizza.getId()+" "+used.size()+ " "+pizza.getIngredients().size()+" "+pizza.getNewIngredients()));
            ...
            for(int y=list.size()-1;y>=0;y--){
                long count = list.get(y).getIngredients().stream()
            ...
            Pizza p = list.get(index);
            list.remove(index);
            bigToSmall.add(p);
            used.addAll(p.getIngredients());
        }
 //       Set<String> m = new HashSet<>();
//        List<Pizza> bts = new ArrayList<>();
//        for(int x=bigToSmall.size()-1;x>=0;x--) {
//            Pizza p = bigToSmall.get(0);
//            m.addAll(p.getIngredients());
//            bts.add(p);
//
//            if (size-x % groupSize==0) {
//                bigToSmall.stream().filter(b -> b.getIngredients().stream()
//                        .filter((s) -> !m.contains(s)).count() > 0).findFirst().ifPresent(pizza -> {
//                    bigToSmall.remove(pizza);
//                    m.addAll(pizza.getIngredients());
//                    bts.add(0, pizza);
//                });
//            }
//
